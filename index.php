...
<table>
      ...
      <tbody>
      <?php
         $sql = $conn->prepare( "SELECT commande.quantite,commande.num_cmd, panier.client_mat,panier.id,produits.nom_prod, produits.prix,produits.nom_prod,users.nom FROM commande
         INNER JOIN panier ON panier.id = commande.panier_id
         INNER JOIN produits ON produits.cod_prod = commande.cod_prod
         INNER JOIN users ON users.mat = panier.client_mat WHERE status IN ('non commander') AND client_mat = '$client_mat'") or die('query failed');
         $sql->execute();
         $grand_total = 0;
         if($sql->rowCount() > 0){ 
            while($row=$sql->fetch()){ ?>
         ...
         <tr>
            ...
            <td>
               <form action="edit_panier.php" method="post">
                  <input type="hidden" name="num_cmd" value="<?php echo $row['num_cmd']; ?>">
            ...
            <td ><a href="sup.php?num_cmd=<?php echo $row['num_cmd'];?>" class="btn-del"> <ion-icon name="trash" color="danger"></ion-icon></td >
         </tr>
...
<?php
         ...
         $grand_total = 0;
            ...
            <form action="cmd.php" method="POST"> 
                   <input type="hidden" name="status" value="commander">
